			...
			for( key in struct1 ){
				if( ListFindNoCase(struct2KeyList, key) > 0 ){
					...
					if ( IsNull(struct1Value) && IsNull(struct2Value) ) {
						...
						var valuesAreBooleans = IsBoolean(struct1Value) AND IsBoolean(struct2Value);
						if (
							(valuesAreBooleans AND struct1Value NEQ struct2Value)
							OR (!valuesAreBooleans AND Compare(struct1Value, struct2Value) NEQ 0)
						) {
							mismatches.success = false;
							mismatches.mismatches[thisPath] = structNew();
							mismatches.mismatches[thisPath].Struct1Value = struct1Value;
